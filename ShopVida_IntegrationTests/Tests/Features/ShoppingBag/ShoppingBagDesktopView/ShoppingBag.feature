...
@Regression
Scenario: Add a product in Shopping Bag and remove it from home page
	...
	When I select "apparel" product category type with "Athleisure" sub category type
	And Verify product subcategory page title should be "Athleisure"
	And I select "Yoga Mat" subcategory type
	...
	#And Select the "White / 2X" size from dropdown
	#Then Verify active dropdown text should be "White / 2X"
	#And Verify "Yoga Mat" should be shown in line items
	When I am on the page "Home"
	Then Verify product card is shown for "Artwork1" with "Yoga Mat" subcategory
	And Remove the product from home page for "Artwork1" with "Yoga Mat" subcategory
	And Verify product card is not shown for "Artwork1" with "Yoga Mat" subcategory
	When I select "Shopping Bag" menu with "" submenu
	Then Verify page heading name should be "Shopping Bag"
...
@Regression
Scenario: Remove an item from shopping bag using Remove Samples Button
...
@Regression
Scenario: Remove an item from shopping bag using minus Button
	When There is no product in bag then add a product
	Then Verify "Proceed to checkout" button should be enabled
